...
func NewApexTrader(zklinkOmniSeeds, apikey, secret, passphrase string) (*ApexTrader, error) {
...
func (t *ApexTrader) normalizeAndStringify(params map[string]interface{}) (string, error) {
...
func (t *ApexTrader) normalize(v interface{}) (interface{}, error) {
	switch val := v.(type) {
...
// sign 对请求参数进行签名
func (t *ApexTrader) sign(params map[string]interface{}, nonce uint64) error {

	return nil
}

// request 发送HTTP请求（带重试机制）
func (t *ApexTrader) request(method, endpoint string, params map[string]interface{}) ([]byte, error) {
	const maxRetries = 3
	var lastErr error
...
func (t *ApexTrader) doRequest(method, endpoint string, params map[string]interface{}) ([]byte, error) {
...
func (t *ApexTrader) OpenShort(symbol string, quantity float64, leverage int) (map[string]interface{}, error) {
	...
	zkSignerMsg, err := t.zklinkSign("open order: " + symbol + " " + qtyStr + " " + priceStr)
	if err != nil {
		return nil, err
	}

	fmt.Println("zkSignerMsg: ", zkSignerMsg)
